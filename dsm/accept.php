...
{
$a1=$_GET['id'];
$result = mysqli_query($link,"SELECT * FROM reg where id=$a1");
$row = mysqli_fetch_array($result);
$m=$row['name'];
$a2=$row['username'];
$a3=$row['password'];
$b=$row['usertype'];
	//echo "insert into login values($a1,'$a2','$a3',0)";
mysqli_query($link,"insert into login values($a1,'$a2','$a3','$b',0)");
mysqli_query($link,"UPDATE reg SET status=1 WHERE id=$a1");

}?>
 <H1><center>Successfully Approved</center></H1>
